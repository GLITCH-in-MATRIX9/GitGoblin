  ...
  return (
    <div className="App bg-primary flex flex-col min-h-screen">
      ...
      <header className="App-header flex items-center justify-center bg-primary">
        <h1
          className="text-9xl font-bold text-primary-content mt-10 mb-10 animate-title text-blue-500"
          style={{ fontFamily: 'Poppins, sans-serif' }}
        >
          {Array.from("Git Goblin").map((char, index) => (
            <span
              key={index}
              className="inline-block transition-all duration-500 hover:text-transparent hover:bg-clip-text hover:bg-gradient-to-r from-purple-400 via-blue-500 to-green-500"
            >
              {char}
            </span>
          ))}
